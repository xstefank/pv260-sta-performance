 ...
 */
public final class NestedDepthReporter extends AbstractCheckReporter {
    ...
    public NestedDepthReporter(int max) {
        this.max = max;
        clearReport();
    }
    ...
    public void visitToken(DetailAST ast) {
        if (depth > max) {
            checkReport = new CheckReport(false, this.getClass().getSimpleName(),
                    String.format("Invalid nesting depth %d (maximum is %d)", depth, max));
        }
        ++depth;
    }
