 ...
 * @author Lars Kühne
 */
public class MethodLengthReporter implements CheckReporter {

    private static final String NAME = "Method length repoter";
    private static final String MESSAGE_FORMAT = "The method length is longer then maximum length: %d > %d";
    ...
    public MethodLengthReporter(int max, boolean countEmpty, FileContents fileContents) {
    ...
    public CheckReport checkPassed(DetailAST ast) {
        ...
        if (openingBrace != null) {
            ...
            if (length > max) {
                return new CheckReport(false, NAME, String.format(MESSAGE_FORMAT, length, max));
            }
        }

        return new CheckReport(true, NAME, "");
    }
    ...
    private int getLengthOfBlock(DetailAST openingBrace, DetailAST closingBrace) {
        ...
        if (!countEmpty) {
            final FileContents contents = fileContents;
